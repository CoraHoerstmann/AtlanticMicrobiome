...
work_dir <- "/scratch1/choerstm/ps113at/data/raw/amplicon_raw_sequences/16S/"
...
#PREFILTERING
filterAndTrim(fnFs.raw,fnFs.preFilt,fnRs.raw,fnRs.preFilt,truncQ=2,minQ=2,minLen=50,maxN=0,multithread = 15)
...
errFWDs <- learnErrors(fnFs.deRep, multithread=8,randomize=TRUE, nbases = 1e8)
errREVs <- learnErrors(fnRs.deRep, multithread=8,randomize=TRUE, nbases = 1e8)
...
dadaFWDs <- dada(fnFs.deRep, err=errFWDs, multithread=8, pool="pseudo")
...
saveRDS(seqtab, file = "seqtabPS113_16S_16S.RDA")
write.csv(t(seqtab), "seqtab_PS113_16S.csv", quote=FALSE )
...
saveRDS(seqtab.nochim, file = "seqtab.nochim.PS113_16S.RDA")
write.csv(t(seqtab.nochim), "seqtab.nochim_PS113_16S.csv", quote=FALSE )
...
write.csv(track, "PS113_16S_DADA2_stats.csv")
...
write(asv_fasta, "PS113_16S_ASVs.fasta")

##outside R, I will use SilvaNGS algorithm to annotate the sequences with following details:
#Sequence type: Illumina MiSeq
#Expected Sequence Quantity: 7178 (grep’>’ PS113_16S_ASVs.fasta| wc -l)
#expected read length: 253 (cat PS113_16S_ASVs.fasta | awk '{if(NR%4==2) print length($1)}' | awk ' { a[i++]=$1; } END { print a[int(i/2)]; }')

#Settings:
#default settings but min quality score set to 20 and for cluster set to 1
